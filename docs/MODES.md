# Viewer modes: Auto, Gutter, Outline
...
Outline, per page, when Gutter's result looks untrustworthy — not on a fixed
schedule, and not for the whole book at once. This is the recommended and
default setting: most pages get Gutter's speed, and the pages Gutter can't
split still come out correctly via Outline.

```mermaid
flowchart LR
    A["Auto"] -->|per page| G["run Gutter"]
    G --> C{"Segmenter.accept()?"}
    C -->|trustworthy| DONE1(["shown as-is"])
    C -->|not trustworthy| O["run Outline"]
    O --> DONE2(["shown"])
...
- **Panel-view button**: while a panel is open, the mode button cycles
  Auto → Gutter → Outline → Auto and immediately re-detects the current page
...
If a page looks wrong, forcing Gutter or Outline via the mode button tells you
...
For most reading, leave it on **Auto** — it's the default for a reason: fast
on the common case, correct on the pages Gutter can't handle. Reach for a
forced mode only when you already suspect which detector is wrong for a
specific book:

- Force **Gutter** if you're reading scans with dark or inverted pages and
  want to confirm panels are still splitting correctly without Outline mode's
  fallback masking a problem.
- Force **Outline** if a book has interlocking or diagonal panel layouts that
  Auto keeps rendering as one undivided region — this skips Gutter's attempt
  entirely instead of waiting for it to fail per page.
- Switch back to **Auto** once you're done diagnosing; a forced mode stays
