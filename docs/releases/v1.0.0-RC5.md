...
| **Feature 4** | **Choose your detector**<br><br>A new *Panel detection* menu lets you pick Automatic, Fast only, or KOReader's detector only, so a page that either one mishandles can be worked around on the spot. Alongside it are toggles for panel pre-rendering and for writing detection timings to the log. |
